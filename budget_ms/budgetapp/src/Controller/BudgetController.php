...
public function addWithMonth(Request $request, string $mois): Response
{
    ...
    // Retrieve the income for the selected month
    ...
    // Create a new Budget entity and set its properties
    $budget = new Budget();
    $budget->setNomBudget($nomBudget);
    $budget->setMontantBudget($montantBudget);
    $budget->setCategories($categories);

    // Associate the budget with the income for the selected month
    $budget->setIncome($income);

    // Persist the new budget entity
    $entityManager = $this->getDoctrine()->getManager();
    $entityManager->persist($budget);
    $entityManager->flush();
...
//get transactions for spesific budget in budget project
